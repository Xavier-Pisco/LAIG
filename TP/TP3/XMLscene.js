...
class XMLscene extends CGFscene {
    ...
    onGraphLoaded() {
        ...
        this.interface.addButtonsGUI();
        this.interface.addGameTypeGUI();
        this.interface.addPastGamesGUI();
        this.updateView();
    ...
    rotateCamera(angle) {
        this.camera.orbit(CGFcameraAxis.Z, angle);
    }
    setCamera() {
        let position;
        if (this.gameOrchestrator.currentPlayer === 'white')
            position = vec3.fromValues(0, 5, 5);
        else
            position = vec3.fromValues(0, 5, -5);
